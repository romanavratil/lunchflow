...
(function () {
  ...
  function buildStyles(cfg) {
    ...
    var isDark = bg[1] <= "5" || bg === "#18181b" || bg === "#1a1a2e"; // rough dark detect
    ...
    return ":host{all:initial;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif}" +
      ...
      ".lf-bd{position:fixed;inset:0;z-index:2147483646;background:rgba(0,0,0,.2);" +
      "opacity:0;pointer-events:none;transition:opacity .25s}" +
      ".lf-bd.open{opacity:1;pointer-events:all}" +

      /* Panel sits above the FAB — anchored bottom:72px so FAB always visible */
      ...
      ".lf-date{font-size:10px;font-weight:600;letter-spacing:.08em;text-transform:uppercase;color:" + sub + ";margin-top:2px}" +
      ...
      ".lf-foot a{color:" + sub + ";text-decoration:none}" +
  ...
  Widget.prototype._renderModal = function(d, cfg) {
    ...
    if (!menu || (!menu.soup && !(menu.mains && menu.mains.length))) {
      ...
      if (menu.soup) {
        ...
        if (menu.soupPrice) soupRow.appendChild(el("div", { className: "lf-rprice" }, menu.soupPrice));
      ...
      if (menu.mains && menu.mains.length) {
        ...
        menu.mains.forEach(function(item) {
          ...
          row.appendChild(el("div", { className: "lf-rprice" }, item.price));
